...
databaseDescribe('chart-of-accounts PostgreSQL foundation', () => {
  ...
  it('automatically and idempotently provisions a complete deterministic hierarchy', async () => {
    ...
    // Chart v1's fourteen, plus v3's customer wallet and v4's promotion cost.
    // v2 added a courier cash receivable and stopped provisioning it when the
    // platform stopped taking money at doors.
    ...
    // fourteen, v3's customer wallet and v4's promotion cost. v2 added a courier
    // cash receivable and stopped adding it when the platform stopped taking
    // money at doors, so a tenant created now records nothing for it.
    expect(await prisma.tenantFinancialBootstrap.count({ where: { tenantId: tenantA } })).toBe(3)
    expect(
  ...
  it('forces RLS for bootstrap, accounts and governance history', async () => {
    ...
    expect(visible[0]).toEqual([{ count: 16n }])
    // One bootstrap row per chart version provisioned. v2 no longer provisions
    // anything, so a tenant created now has v1's, v3's and v4's.
    expect(visible[1]).toEqual([{ count: 3n }])
  })
})
