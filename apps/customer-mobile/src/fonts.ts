...
export function useAppFonts(): boolean {
   ...
   *
   * An error here is treated as ready on purpose. A shop that never paints
   * because a typeface failed to decode is worse in every way than one that
   * paints in the system font: the customer can still order bread, and the only
   * thing lost is the one thing that was never worth a blank screen.
   */
