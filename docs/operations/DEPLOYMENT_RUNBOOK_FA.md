...
- Ubuntu 24.04 LTS (یا Debian 12). 22.04 هم کار می‌کند، ولی پشتیبانی استانداردش
  آوریل ۲۰۲۷ تمام می‌شود — سروری که امروز راه می‌افتد نباید سال بعد جابه‌جا شود.
- Node.js **26.3 یا بالاتر** — همان نسخهٔ `.node-version` که CI روی آن اجرا
  می‌شود و `engines` در `package.json` می‌خواهد. (`target: 'node22'` در
  `apps/api/scripts/bundle.ts` کف نحوی خروجی است، نه نسخهٔ اجرا.)
- PostgreSQL 16
- pnpm 11
- nginx و certbot

### سخت‌افزار — سنجیده، نه حدس زده

روی بستهٔ تولیدی (`node dist/server.js` و `node server.js` سایت)، با نقش محدود
`alo_noon_app` و ۶۰۰ بازدید صفحه با ۲۰ درخواست هم‌زمان (شهریور ۱۴۰۵):

| چیز                       | بیکار  | زیر بار              |
| ------------------------- | ------ | -------------------- |
| API                       | ~۲۰۰MB | ~۳۶۰MB               |
| سایت (Next.js)            | ~۱۳۰MB | ~۳۲۰MB               |
| PostgreSQL (پیش‌فرض)      | ~۱۶۰MB | —                    |
| ساخت (`pnpm build`) — اوج | +۱٫۰GB | ۱۷۱ ثانیه روی ۴ هسته |

**ساخت روی همین سرور و کنار سرویس‌های در حال اجرا انجام می‌شود** (گام ۲)، پس
حافظه باید هر دو را با هم جا بدهد: حدود ۱٫۴GB برای سرویس‌ها زیر بار به‌علاوهٔ
۱GB برای ساخت، به‌علاوهٔ سیستم‌عامل و nginx.

|          | حداقل          | پیشنهادی    |
| -------- | -------------- | ----------- |
| پردازنده | ۲ vCPU         | ۲ تا ۴ vCPU |
| حافظه    | ۴GB + ۲GB swap | ۴ تا ۸GB    |
| دیسک     | ۴۰GB SSD       | ۶۰GB NVMe   |

۲GB حافظه **کافی نیست**: ساخت در وسط استقرار با کمبود حافظه کشته می‌شود یا سرویس
زنده را به swap می‌راند. دیسک برای دو نسخه کنار هم (عقب‌گرد)، مخزن pnpm (~۲GB)،
خروجی ساخت سایت (~۱GB) و دو هفته پشتیبان شبانه است.
...
**`prisma generate` اختیاری نیست.** کلاینت Prisma کد تولیدشده است و تنها چیزی
است که از بستهٔ ساخت بیرون می‌ماند؛ از نسخهٔ ۷ در
`packages/database/generated/client` ساخته می‌شود، نه در `node_modules`. بدون
این گام سرویس بالا نمی‌آید. `pnpm build` خودش هم آن را می‌سازد — این خط برای
وقتی است که فقط بخشی از مراحل اجرا می‌شود.
...
| `API_PORT`                                                      | `3001`               |                                                                                                                     |
| `API_TRUST_PROXY`                                               | `loopback`           | nginx روی همین سرور است. آدرس پروکسی، نه تعداد آن؛ حدس‌زدنش محدودیت نرخ و شمارش سوءاستفادهٔ OTP را از کار می‌اندازد |
...
`WEB_BASE_URL` استثناست و باید **آدرس عمومی با https** باشد، نه loopback. سه چیز
با آدرس نسبی کار نمی‌کنند و همگی از این متغیر می‌خوانند: `robots.txt` که باید
نشانی sitemap را کامل بنویسد، خودِ `sitemap.xml` که ورودی‌هایش طبق استاندارد
مطلقند، و `metadataBase` که نشانی تصویر Open Graph را می‌سازد.

> [!WARNING] **نبودنش خرابیِ ساکت است.** اگر تنظیم نشود، مقدار
> `http://localhost:3000` جایگزین می‌شود: سایت بالا می‌آید، هیچ‌چیز قرمز
> نمی‌شود، ولی `robots.txt` یک sitemap روی localhost معرفی می‌کند که هیچ موتور
> جست‌وجویی نمی‌تواند بخواند، و هر لینکی که کسی از فروشگاه به اشتراک بگذارد بدون
> تصویر باز می‌شود. هفته‌ها بعد و از روی نتایج جست‌وجو معلوم می‌شود، نه از روی
> صفحه.
...
الو نون به‌عنوان اپ روی صفحهٔ اصلی نصب می‌شود، و مرورگر این را در سکوت تصمیم
می‌گیرد: اگر چیزی سر جایش نباشد، فقط گزینهٔ نصب هرگز ظاهر نمی‌شود. هیچ خطایی در
کار نیست. پس بعد از اینکه TLS بالا آمد، از **بیرون** سرور بررسی کنید:

```bash
curl -fsS https://<domain>/manifest.webmanifest | jq '{id, start_url, display, icons: (.icons|length)}'
curl -s -o /dev/null -w 'sw %{http_code}\n'   https://<domain>/sw.js
curl -s -o /dev/null -w 'icon %{http_code}\n' https://<domain>/brand/icon-maskable-512.png
curl -s https://<domain>/robots.txt | grep -i sitemap
```

- مانیفست باید `"display": "standalone"` و چهار آیکون داشته باشد.
- `sw.js` باید ۲۰۰ بدهد؛ بدون آن نه نصب‌پذیری هست نه صفحهٔ آفلاین.
- خط `Sitemap:` باید دامنهٔ واقعی را نشان بدهد، نه `localhost` — اگر localhost
  بود، `WEB_BASE_URL` تنظیم نشده.

> [!NOTE] نصب فقط روی **https** کار می‌کند (یا `localhost`). تا وقتی گام ۶ تمام
> نشده، این بررسی‌ها معنا ندارند.
...
- [ ] `API_TRUST_PROXY=loopback` (یا IP/CIDR پروکسی) و nginx هر سه هدر
      `X-Forwarded-*` را می‌فرستد
