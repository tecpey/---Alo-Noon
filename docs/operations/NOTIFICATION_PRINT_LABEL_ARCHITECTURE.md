...
این سند معماری اعلان چندنقشی، چاپ خودکار سفارش و لیبل بسته‌بندی را برای مشتری، نانوایی، پیک و تیم عملیات تعریف می‌کند. این قابلیت بخشی از هسته سفارش و fulfillment است و نباید به‌صورت افزونه نمایشی یا وابسته به یک چاپگر خاص ساخته شود.
...
1. **Transactional**: مستقیماً ناشی از رویداد سفارش و غیرقابل خاموش‌کردن در موارد حیاتی.
...
1. سفارش تنها پس از عبور از validation، رزرو ظرفیت و وضعیت معتبر مالی به `CONFIRMED` می‌رسد.
...
ارسال دستی نیازمند permission، ثبت Audit Log و محدودیت نرخ است. پیام‌های انبوه حساس می‌توانند نیازمند تایید دوم باشند.
