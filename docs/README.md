...
> اسناد هدف و معماری آینده را از قابلیت‌های پیاده‌سازی‌شده تفکیک می‌کنند. وضعیت واقعی Phase 1 در `architecture/README.md`، `architecture/DOMAIN_BOUNDARIES.md` و اسناد محصول انگلیسی ثبت شده است.
...
ریپو در Phase 1 Domain Foundation است. مدل دامنه و دیتابیس، قراردادهای نسخه‌دار، قواعد سفارش/تازگی، Migration، Outbox و Audit foundation همراه تست‌ها پیاده‌سازی شده‌اند. احراز هویت و RBAC عملیاتی، Commerce تراکنشی، پرداخت، اعلان delivery و چاپ اجرایی هنوز برنامه‌ریزی‌شده‌اند و نباید قابلیت تحویل‌شده معرفی شوند.
