...
This matrix classifies current and near-term aggregates. `Tenant-owned` means
every persistent row and derived artifact carries `tenantId`. `Global reference`
is read-only to tenants and changed only by platform authority.
...
A new table, event or object-store artifact cannot be introduced until its row
in this matrix is added and reviewed.
