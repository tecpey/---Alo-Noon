...
الو نون صرفاً یک فروشگاه اینترنتی یا سرویس پیک نان نیست. این پروژه یک پلتفرم
API-first و Persian-first برای مدیریت یکپارچه تجربه مشتری، نانوایی‌ها، محصولات،
پیش‌سفارش، اشتراک، تولید، بسته‌بندی، چاپ لیبل، پیک، مسیریابی، اعلان، CRM و
عملیات شهری است.

## مأموریت

ایجاد قابل‌اعتمادترین و ساده‌ترین راه دسترسی خانواده‌ها به نان‌های
بسته‌بندی‌شده، نان‌های تازه امضادار، محصولات رژیمی، حجیم، کیک، دسر و ساندویچ‌های
ویژه؛ همراه با کنترل کیفیت، زمان‌بندی تولید، رهگیری سفارش و تحویل قابل پیش‌بینی.

## مدل محصول

### ۱. نان‌های سنتی بسته‌بندی‌شده

نان‌های بربری، سنگک، تافتون و ساندویچی سنتی با بسته‌بندی اختصاصی الو نون و عرضه
در ساعات گسترده:
...
محصولات اختصاصی هر نانوایی، مانند سنگک دو رو کنجدی، بربری کنجدی و سیاه‌دانه،
بربری تبریزی و سایر محصولات محدود:
...
الو نون نباید به یک سوپرمارکت عمومی تبدیل شود؛ هر دسته جدید باید با جایگاه تخصصی
برند، استاندارد کیفیت و توان عملیاتی پلتفرم هم‌راستا باشد.
...
پس از تایید معتبر سفارش، سامانه یک رویداد قابل تکرار و قابل ممیزی ایجاد می‌کند.
سرویس چاپ نانوایی بر اساس سیاست شعبه، موارد زیر را چاپ می‌کند:
...
چاپ باید idempotent باشد؛ چاپ مجدد ثبت شود و از تولید بی‌نهایت لیبل در اثر Retry
جلوگیری شود. خرابی چاپگر نباید سفارش را گم کند و باید در پنل عملیات هشدار، صف
Retry و مسیر چاپ دستی کنترل‌شده وجود داشته باشد.

جزئیات کامل در
[Notification, Print & Label Architecture](docs/operations/NOTIFICATION_PRINT_LABEL_ARCHITECTURE.md)
ثبت شده است.
...
- Modular Monolith تا زمانی حفظ می‌شود که شواهد مقیاس استخراج سرویس را توجیه
  کند.
...
| `packages/domain`        | قواعد مستقل دامنه و invariantهای کسب‌وکار | —                          |
...
Batch delivery ده‌مقصدی از ابتدا در مدل دامنه دیده می‌شود، اما فعال‌سازی خودکار
آن پس از پایدارشدن تحویل تک‌سفارش انجام خواهد شد.
...
ریپو در **Phase 2E Checkout Acceptance** قرار دارد. علاوه بر زیرساخت Phase 0 و
Domain Foundation، API شهرهای فعال، کاتالوگ و محدوده سرویس، OTP و Session امن،
RBAC، Cart سروری وابسته به مشتری، نشانی تحویل وابسته به مشتری، قیمت‌گذاری قطعی
ارسال، Quote زمان‌دار و تبدیل اتمیک Quote به Order همراه با رزرو پایدار ظرفیت
پیاده‌سازی شده‌اند. بنیاد داخلی Payment و دفترکل دوطرفه نیز بدون اتصال درگاه
فراهم است. اتصال ارائه‌دهنده تأییدشده پیامک، اجرای درگاه پرداخت، بازپرداخت و
تسویه، پنل‌های کامل، Dispatch، ارسال اعلان و اجرای چاپ هنوز باید در فازهای بعدی
ساخته شوند.
...
- `GET|POST /api/v1/addresses` برای نشانی‌های تحویل مشتری
- `GET /api/v1/cart` و `PUT|DELETE /api/v1/cart/items/:offeringId` برای سبد
  سروری
- `POST /api/v1/cart/quote` برای قیمت قطعی و snapshot تحویل
- `POST /api/v1/orders` برای پذیرش اتمیک Quote و رزرو ظرفیت
...
- [مرزهای دامنه](docs/architecture/DOMAIN_BOUNDARIES.md)
- [امنیت](SECURITY.md)
- [مشارکت](CONTRIBUTING.md)

## حقوق مالکیت

این ریپو خصوصی، محرمانه و نرم‌افزار اختصاصی الو نون است. هیچ مجوزی برای کپی،
انتشار، استقرار یا ایجاد اثر مشتق بدون مجوز کتبی مالک صادر نشده است.
